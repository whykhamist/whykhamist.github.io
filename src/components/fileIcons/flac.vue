<template>
  <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 96">
    <path
      class="st125"
      d="M0 2v92c0 1.1.9 2 2 2h68c1.1 0 2-.9 2-2V28H50c-1.1 0-2-.9-2-2V0H2C.9 0 0 .9 0 2z"
    />
    <path class="st125" d="M71.9 24c-.1-.4-.2-.7-.4-1L52 1.2V24h19.9z" />
    <path
      class="st426"
      d="M6 41h60v49H6zm.6-34.6h6.8v2.9H9.7v3.1h3v2.9h-3v5.4H6.6V6.4zm8.1 0h3.1v11.4h4.1v2.9h-7.2V6.4z"
    />
    <path
      class="st426"
      d="M25.4 6.4h3.4l3.4 14.3H29l-.6-3h-2.9l-.6 3h-3.1l3.6-14.3zM28 15l-.5-2.8c-.2-.9-.4-2.5-.4-2.5H27s-.2 1.5-.4 2.5l-.5 2.8H28zm10.4-8.9c1.2 0 2.5.4 3.4 1.2l-1.3 2.8c-.4-.5-1.2-.9-2-.9-2 0-3.1 1.9-3.1 4.2 0 2.3 1.1 4.4 3 4.4.9 0 1.6-.5 2.2-1l1.3 2.8c-.9.7-1.9 1.3-3.7 1.3-3.8 0-6-3.1-6-7.4.1-4.2 2.4-7.4 6.2-7.4z"
    />
    <path
      class="st125"
      d="M47 66c-3.1-1.2-6.7.3-8 3.4-1.2 3.1.3 6.7 3.4 8 .6.3 1.3.4 2 .4 3.5.1 6.4-2.6 6.4-6V47.9c-.2 0-.3.1-.5.1-6.8 1.9-13.5 3.9-20.3 5.8-.6.2-.6.5-.6 1v16.8c-.5-.2-.7-.3-1.1-.4-2.5-.9-4.7-.4-6.5 1.4-1.8 1.7-2.3 4.3-1.3 6.6 1.3 3.1 4.9 4.6 8 3.4 2.3-1 3.8-3.2 3.8-5.7V61.3c0-.7.2-.9.9-1.1 3.3-.9 6.6-1.9 10-2.8L48 56v10.4c-.5-.2-.7-.3-1-.4z"
    />
  </svg>
</template>

<style scoped>
.st125 {
  fill: #039;
}
.st426 {
  fill: #fff;
}
</style>
