<template>
  <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 96">
    <path
      class="st323"
      d="M0 2v92c0 1.1.9 2 2 2h68c1.1 0 2-.9 2-2V28H50c-1.1 0-2-.9-2-2V0H2C.9 0 0 .9 0 2z"
    />
    <path class="st323" d="M71.9 24c-.1-.4-.2-.7-.4-1L52 1.2V24h19.9z" />
    <path
      d="M6 41h60v49H6zm1.2-24.3c.5.6 1.3 1.2 2.2 1.2.8 0 1.2-.6 1.2-1.2 0-2-4.5-2.1-4.5-6.2 0-2.4 1.6-4.3 4.3-4.3 1.1 0 2.3.3 3.2 1l-1.1 2.9c-.5-.4-1.4-.9-2.1-.9-.9 0-1.3.6-1.3 1.2 0 2.1 4.5 2 4.5 6.1 0 2.3-1.4 4.4-4.2 4.4-1.3 0-2.7-.5-3.8-1.4l1.6-2.8zm10.7-7.4h-3V6.4H24v2.9h-3v11.4h-3.1V9.3zm7.8-2.9h6.8v2.9h-3.7V12h2.9v2.9h-2.9v2.9h3.9v2.9h-7V6.4zm9 0H39c2.5 0 4.1 2 4.1 4.9 0 2.9-1.7 5-4.1 5h-1.2v4.4h-3.1V6.4zm3.7 7c1 0 1.5-.9 1.5-2.1s-.6-2-1.5-2h-.6v4.1h.6z"
      fill="#fff"
    />
    <path
      d="M51.6 65.9c-.5-.2-1.1 0-1.3.5-1.1 2.5-2.9 4.7-5.1 6.4l-6.7-13.6c1-.8 1.6-2 1.6-3.4 0-2.1-1.5-3.9-3.5-4.3v-3.4c0-.5-.4-1-1-1-.5 0-1 .4-1 1v3.4c-2 .4-3.5 2.2-3.5 4.3 0 1.4.6 2.6 1.6 3.4L26 72.8c-2.2-1.7-4-3.8-5.1-6.4-.2-.5-.8-.7-1.3-.5-.5.2-.7.8-.5 1.3 1.3 3 3.4 5.4 6 7.3l-4 8c-.2.5 0 1.1.4 1.3.1 0 .3.1.4.1.3 0 .7-.2.9-.5l3.8-7.8c2.7 1.5 5.6 2.2 8.7 2.2s6-.8 8.7-2.2l3.8 7.8c.1.3.5.5.9.5.1 0 .3 0 .4-.1.5-.2.7-.8.4-1.3l-3.9-8c2.6-1.8 4.7-4.4 6-7.3.6-.5.4-1.1 0-1.3zm-16-12.6c1.4 0 2.5 1.1 2.5 2.5s-1.1 2.5-2.5 2.5-2.5-1.1-2.5-2.5 1.2-2.5 2.5-2.5zm0 22.6c-2.8 0-5.4-.7-7.8-2l6.8-13.7c.3.1.7.1 1.1.1s.7 0 1.1-.1l6.8 13.7c-2.5 1.3-5.2 2-8 2z"
      fill="#369"
      stroke="#369"
      stroke-width="1.5"
      stroke-miterlimit="10"
    />
  </svg>
</template>

<style scoped>
.st323 {
  fill: #369;
}
</style>
