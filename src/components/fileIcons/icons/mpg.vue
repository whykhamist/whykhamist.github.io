<template>
  <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 96">
    <path
      class="st227"
      d="M0 2v92c0 1.1.9 2 2 2h68c1.1 0 2-.9 2-2V28H50c-1.1 0-2-.9-2-2V0H2C.9 0 0 .9 0 2z"
    />
    <path class="st227" d="M71.9 24c-.1-.4-.2-.7-.4-1L52 1.2V24h19.9z" />
    <path
      d="M6 41h60v49H6zM7 6.4h3.5l1.3 6c.2.9.4 2.4.4 2.4h.1s.2-1.5.4-2.4l1.3-6h3.5l.9 14.3h-2.9l-.5-6.4c-.1-1.1 0-2.4 0-2.4h-.1s-.2 1.5-.4 2.4l-.8 4.1h-3l-.8-4.1c-.2-.9-.4-2.4-.4-2.4h-.1s.1 1.3 0 2.4L9 20.7H6.1L7 6.4zm14.3 0h4.3c2.5 0 4.1 2 4.1 4.9 0 2.9-1.7 5-4.1 5h-1.2v4.4h-3.1V6.4zm3.7 7c1 0 1.5-.9 1.5-2.1s-.6-2-1.5-2h-.6v4.1h.6zm12.9-7.3c1.3 0 2.8.4 3.7 1.3l-1.3 2.8c-.5-.6-1.3-.9-2.2-.9-2.1 0-3 1.9-3 4.2 0 2.6 1.1 4.3 2.7 4.3.6 0 1.2-.2 1.6-.6v-1.7H38v-2.9h4.1v8.1h-2.4v-.9c-.5.7-1.5 1.2-2.6 1.2-2.6 0-5.3-2.7-5.3-7.3.1-4.5 2.5-7.6 6.1-7.6z"
      fill="#fff"
    />
    <path
      class="st227"
      d="M13 83.1h46V47.9H13v35.2zm8.9-2.7h-5.5v-3.9h5.5v3.9zm16.9 0h-5.5v-3.9h5.5v3.9zm11.3-30.1h5.5v4h-5.5v-4zm0 26.1h5.5v3.9h-5.5v-3.9zm-8.5-26.1H47v3.9h-5.5v-3.9zm0 26.1H47v3.9h-5.5l.1-3.9zm-8.4-26.1h5.5v3.9h-5.5v-3.9zm-.8 9.7l7.2 4.1c1.3.8 1.3 2 0 2.8L32.4 71c-1.3.8-2.4.1-2.4-1.4v-8.3c0-1.4 1.1-2.1 2.4-1.3zm-7.7-9.7h5.4v3.9h-5.4v-3.9zm0 26.1h5.4v3.9h-5.4v-3.9zm-8.3-26.1h5.5v4h-5.5v-4z"
    />
  </svg>
</template>

<style scoped>
.st227 {
  fill: #f60;
}
</style>
