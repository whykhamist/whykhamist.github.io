<template>
  <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 96">
    <path
      class="st179"
      d="M0 2v92c0 1.1.9 2 2 2h68c1.1 0 2-.9 2-2V28H50c-1.1 0-2-.9-2-2V0H2C.9 0 0 .9 0 2z"
    />
    <path class="st179" d="M71.9 24c-.1-.4-.2-.7-.4-1L52 1.2V24h19.9z" />
    <path
      d="M6 41h60v49H6zm.6-34.6h3.3v5.5h.6l2.3-5.5h3.5l-2.9 6.8 3 7.4h-3.7l-2.3-5.8h-.5v5.8H6.6V6.4zm12.5 0h7.1v2.9h-3.8v3.1h3.1v2.9h-3.1v5.4h-3.3V6.4zm11.4 6.9c-.8-.8-1.4-1.8-1.4-3.3 0-2.1 1.2-3.9 4-3.9 2.7 0 4 1.6 4 3.6 0 1.5-1 3-1.4 3.6.9.7 1.7 1.8 1.7 3.4 0 2.2-1.4 4.2-4.4 4.2-3.1 0-4.3-2-4.3-4 0-1.9 1.4-3.2 1.8-3.6zm2.1 1.4c-.3.4-.6 1-.6 1.8s.2 1.5 1.1 1.5c.7 0 1-.4 1-1.2.1-1.2-.8-1.7-1.5-2.1zm.9-2.6c.2-.3.4-.9.4-1.7 0-.9-.3-1.2-.9-1.2s-.7.4-.7 1c0 1 .6 1.5 1.2 1.9z"
      fill="#fff"
    />
    <path
      id="path1236"
      class="st179"
      d="M56.6 61.3c-.2-.5-.6-.9-1-1.3-.1.4-.2.8-.5 1.2L42.6 80.5c-.5.7-1.5.9-2.3.7l-20-5.6c-1.2-.3-2.6-1-2.7-2.4 0-.5 0-.7.3-.9.3-.2.6-.2.9-.1l18.8 5.2c2.7.8 3.5.2 5.5-2.8L54.5 57c.6-.9.7-2 .4-2.9-.3-.9-1.1-1.6-2.1-1.9l-16.5-4.6c-.4-.1-.8-.1-1.2-.1v-.1c-2.5-1.5-3.5 1.3-4.8 2.4-.5.4-1.1.7-1.3 1.1-.2.4-.1.8-.2 1.1-.5 1.1-1.9 2.9-2.6 3.4-.4.3-1 .4-1.3.9-.2.3-.2.9-.3 1.3-.4 1-1.7 2.7-2.6 3.5-.3.3-.8.5-1 .9-.2.3-.1.9-.4 1.2-.6 1-1.8 2.6-2.7 3.5-.5.5-1.1.7-1.3 1.2-.1.2 0 .6-.1.9-.2.5-.4.9-.5 1.3-.5.7-.8 1.7-.7 2.7.2 2.4 2 4.8 4.2 5.4l20 5.6c1.9.5 4.2-.4 5.2-2l11.5-17.7c.6-.8.7-1.9.4-2.8zm-23.1-6.4l.8-1.2c.2-.3.7-.5 1.1-.4L48.6 57c.4.1.5.5.3.8l-.8 1.2c-.2.3-.7.5-1.1.4l-13.2-3.7c-.4-.1-.5-.4-.3-.8zm-3.3 5l.8-1.2c.2-.3.7-.5 1.1-.4L45.3 62c.4.1.5.5.3.8l-.8 1.2c-.2.3-.7.5-1.1.4l-13.2-3.7c-.4-.1-.6-.5-.3-.8z"
    />
  </svg>
</template>

<style scoped>
.st179 {
  fill: #963;
}
</style>
