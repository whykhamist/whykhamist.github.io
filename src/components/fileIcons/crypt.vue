<template>
  <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 96">
    <path
      class="st75"
      d="M0 2v92c0 1.1.9 2 2 2h68c1.1 0 2-.9 2-2V28H50c-1.1 0-2-.9-2-2V0H2C.9 0 0 .9 0 2z"
    />
    <path class="st75" d="M71.9 24c-.1-.4-.2-.7-.4-1L52 1.2V24h19.9z" />
    <path
      d="M14.8 70.2c1.3 0 2.6.4 3.6 1.2L17 74.2c-.4-.5-1.3-.9-2.1-.9-2.1 0-3.2 1.9-3.2 4.2 0 2.3 1.2 4.4 3.2 4.4 1 0 1.7-.5 2.3-1l1.4 2.8c-.9.7-2 1.3-3.9 1.3-4 0-6.3-3.1-6.3-7.4-.1-4.4 2.3-7.4 6.4-7.4zm6 .2h4.5c.6 0 1.3.1 1.9.4 1.4.6 2.4 2.1 2.4 4 0 1.7-.8 3.3-2 4 0 0 .2.2.5.9l2.3 5h-3.6l-2-4.8H24v4.8h-3.3l.1-14.3zm3.9 6.6c.9 0 1.6-.7 1.6-1.9 0-.7-.2-1.2-.6-1.5-.3-.2-.6-.3-.9-.3h-.7V77h.6zm9.9 1.8L31 70.4h3.6l1.1 3.4c.3 1 .5 2.1.5 2.1h.1s.2-1.1.5-2.1l1.1-3.4h3.6l-3.6 8.4v5.9h-3.3v-5.9zm8.8-8.4h4.5c2.6 0 4.4 2 4.4 4.9s-1.7 5-4.4 5h-1.2v4.4h-3.3V70.4zm3.9 7c1 0 1.6-.9 1.6-2.1s-.7-2-1.6-2h-.7v4.1h.7zm9.6-4.1h-3.1v-2.9h9.6v2.9h-3.2v11.4h-3.3V73.3z"
      fill="#666"
    />
    <path
      class="st419"
      d="M0 94V2C0 .9.9 0 2 0h48c.6 0 1.1.2 1.5.7l20 22.3c.3.4.5.8.5 1.3V94c0 1.1-.9 2-2 2H2c-1.1 0-2-.9-2-2zm68-68.9L49.1 4H4v88h64V25.1z"
    />
    <path
      class="st419"
      d="M48 26V4c0-1.1.9-2 2-2s2 .9 2 2v20h16c1.1 0 2 .9 2 2s-.9 2-2 2H50c-1.1 0-2-.9-2-2zm1.9 13.2h-2.6v-5c0-5.7-4.6-10.2-10.2-10.2-5.7 0-10.2 4.6-10.2 10.2v5h-2.6c-1 0-1.8.8-1.8 1.8v17.4c0 1 .8 1.8 1.8 1.8h25.6c1 0 1.8-.8 1.8-1.8V40.9c0-.9-.8-1.7-1.8-1.7zM39.3 53.6c.2.6-.3 1.1-.9 1.1h-2.7c-.6 0-1-.6-.9-1.1l.8-3.2c-1-.5-1.7-1.5-1.7-2.8 0-1.7 1.4-3.1 3.1-3.1s3.1 1.4 3.1 3.1c0 1.2-.7 2.3-1.7 2.8l.9 3.2zm2.9-14.4H32v-5c0-2.8 2.3-5.1 5.1-5.1s5.1 2.3 5.1 5.1v5z"
    />
  </svg>
</template>

<style scoped>
.st75 {
  fill: #fff;
}
.st419 {
  fill: #bababa;
}
</style>
