<template>
  <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 96">
    <path
      class="st113"
      d="M0 2v92c0 1.1.9 2 2 2h68c1.1 0 2-.9 2-2V28H50c-1.1 0-2-.9-2-2V0H2C.9 0 0 .9 0 2z"
    />
    <path class="st113" d="M71.9 24c-.1-.4-.2-.7-.4-1L52 1.2V24h19.9z" />
    <path
      d="M6 41h60v49H6zm.6-34.6h7.2v2.9H9.9V12H13v2.9H9.9v2.9H14v2.9H6.6V6.4zm10.5 0h3.3v11.4h4.3v2.9h-7.6V6.4zm10.3 0h7.1v2.9h-3.8v3.1h3.1v2.9h-3.1v5.4h-3.3V6.4z"
      fill="#fff"
    />
    <path
      class="st113"
      d="M49 49.8H23.5v3.5h-5.2v1.1h-2.8v1h2.8v1.3h5.2v6.1h-5.2V64h-2.8v1h2.8v1.1h5.2v6.6h-5.2v1.2h-2.8v1h2.8V76h5.2v4.7H49V76h4.7v-1.2h2.8v-1h-2.8v-1.2H49V66h4.7v-1h2.8v-1h-2.8v-1.3H49v-6.1h4.7v-1.3h2.8v-1h-2.8v-1.1H49v-3.4zm-4.9 8.3c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2c0 1.2-.9 2-2 2z"
    />
  </svg>
</template>

<style scoped>
.st113 {
  fill: #999;
}
</style>
